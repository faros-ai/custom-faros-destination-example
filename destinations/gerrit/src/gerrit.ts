...
export class Gerrit extends Converter {
  source = "Gerrit";
  ...
  initialize(ctx: StreamContext): void {
    ...
    if (!gerritConfig?.organization) {
      throw new Error(
        "Organization must be configured in source_specific_configs.gerrit.organization"
      );
    }
  ...
  async convert(
    ...
    const event = record.record.data.value as GerritEvent;
    const results: DestinationRecord[] = [];

    switch (event.type) {
      case "patchset-created":
        results.push(
          ...this.convertPatchSetCreated(event as PatchSetCreatedEvent)
        );
        break;
      case "reviewer-added":
      ...
      case "reviewer-deleted":
        results.push(
          ...this.convertReviewerDeleted(event as ReviewerDeletedEvent)
        );
        break;
      case "comment-added":
      ...
      case "change-abandoned":
        results.push(
          ...this.convertChangeAbandoned(event as ChangeAbandonedEvent)
        );
        break;
      case "change-merged":
      ...
      case "wip-state-changed":
        results.push(
          ...this.convertWipStateChanged(event as WipStateChangedEvent)
        );
        break;
      case "change-restored":
        results.push(
          ...this.convertChangeRestored(event as ChangeRestoredEvent)
        );
        break;
    }
  ...
  private createCommonRecords(
    change?: GerritChange,
    patchSet?: GerritPatchSet,
    eventCreatedOn?: number,
    isWipStateChange?: boolean
  ): {
    ...
    if (change && repoKey) {
      ...
      const closedAt =
        change.status === "ABANDONED" || change.status === "MERGED"
          ? this.secondsToDate(eventCreatedOn)
          : null;
      const mergeCommit =
        change.status === "MERGED" && patchSet
          ? {
      ...
      if (isWipStateChange) {
        ...
        readyForReviewAt = !change.wip
          ? this.secondsToDate(eventCreatedOn)
          : null;
      } else {
        // For other events: if not WIP, use change creation date
        readyForReviewAt = !change.wip
          ? this.secondsToDate(change.createdOn)
          : null;
      }
  ...
  private convertPatchSetCreated(
  ...
  private collectRepository(
    project?: string,
    orgKey?: { uid: string; source: string }
  ): RepoKey | null {
    if (!project || !orgKey) {
      return null;
    }
  ...
  private convertReviewerAdded(event: ReviewerAddedEvent): DestinationRecord[] {
    const { records, prKey } = this.createCommonRecords(
      event.change,
      event.patchSet
    );
  ...
  private convertReviewerDeleted(
    event: ReviewerDeletedEvent
  ): DestinationRecord[] {
    const { records, prKey } = this.createCommonRecords(
      event.change,
      event.patchSet
  ...
  private convertCommentAdded(event: CommentAddedEvent): DestinationRecord[] {
    ...
    // We do not have review number so we use the eventCreatedOn as the review number
  ...
  private convertVoteDeleted(event: VoteDeletedEvent): DestinationRecord[] {
    ...
    const codeReviewDeleted = event.approvals?.find(
      (a) => a.type === "Code-Review"
    );
  ...
  private convertChangeAbandoned(
    event: ChangeAbandonedEvent
  ): DestinationRecord[] {
    const { records } = this.createCommonRecords(
      event.change,
      event.patchSet,
  ...
  private convertWipStateChanged(
  ...
  private convertChangeRestored(
    event: ChangeRestoredEvent
  ): DestinationRecord[] {
    const { records } = this.createCommonRecords(
      event.change,
      event.patchSet,
